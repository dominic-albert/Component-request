...
CREATE TABLE IF NOT EXISTS users (
    ...
    name VARCHAR(255),
    role VARCHAR(50) DEFAULT 'Requester' CHECK (role IN ('Requester', 'Creator', 'Admin')),
...
CREATE TABLE IF NOT EXISTS component_requests (
    ...
    status VARCHAR(50) DEFAULT 'Pending' CHECK (status IN ('Pending', 'In Progress', 'Completed', 'Denied')),
    denial_reason TEXT,
    ...
    source VARCHAR(20) DEFAULT 'manual' CHECK (source IN ('manual', 'figma-plugin')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
